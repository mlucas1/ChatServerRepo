...
public class ChatServerThread extends Thread {
    ...
    public ChatServerThread(Socket socket) {
        ...
        try {
            ...
            /* Welcome the user */
<<<<<<< HEAD
            this.out.println( "<System> Welcome to the chat server antechamber! You are " + "<" + username + ">" + "\n"
            				+ "<System> To create a new chatroom, type:		\"/newRoom [room name]\"\n"
            				+ "<System> To join a chatroom, type: 			\"/join [room name]\"\n"
            				+ "<System> To change your nickname, type:			\"/nick [name]\"\n"
            				+ "<System> To disconnect from the chat server,type: 	\"/disconnect\"");
=======
            this.out.println("<System> Welcome to the chatroom! You are " + "\"" + username + "\"" + ". There are " + (room.clients.size()+1) + " total users in the room.");
>>>>>>> cf543fa22812fcf69b2a50417239efe1e850f147
    ...
    private synchronized boolean setNickname(String s) {
    	...
    	if(username != null) {
    		if(canMessage) {
    			...
    			this.out.println("<System> " + username + "'s nickname is now " + "\"" + s + "\"");
    	}
        username=s;
        return true;
    }
